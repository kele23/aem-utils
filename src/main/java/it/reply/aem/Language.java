...
class Language {
    ...
    //(:4 => Key, :6 => Message)
    private static final String XML_LANGUAGE_NODE_EXPRESSION = "<.*\\s*jcr:mixinTypes=(['\"])" +
            "\\[sling:Message]\\1\\s*jcr:primaryType=([\"'])nt:folder\\2\\s*sling:key=([\"'])([^\"']*)" +
            "\\3\\s*sling:message=([\"'])([^\"']*)\\5\\/>";
    ...
    void commit() throws IOException {
        ...
        for (String key : map.keySet()) {
            String nodeName = key.replaceAll("[^\\p{ASCII}]", "").replaceAll("[^a-zA-Z0-9]", "");
